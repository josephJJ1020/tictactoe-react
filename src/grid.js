...
function Grid(props) {
    ...
    useEffect(() => {
        if (isInitialMount.current) {
            ...
            console.log(`current grid in Grid: ${props.board.grid}`)
        }
    }, [props.board])
    */
    ...
    useEffect(() => {
        ...
        if (isInitialMount.current) {
            ...
            /*
            // get computer player move based on new board
            var computer_move = getMove();
    
            // display computer move
            activeTiles[computer_move].innerHTML = props.first === 'X' ? 'O' : 'X';
    
            // check if computer player wonnered
            if (wonnered()) {
                return;
            }
    
            // update board
            var activeTilesNew = document.getElementsByClassName(`${styles.tile}`);
            var activeTilesNew = [...activeTilesNew];
    
            var newGrid1 = []
            activeTilesNew.map((square) => {newGrid1.push(square.innerHTML)});
    
            updateBoard(newGrid1);
            */
